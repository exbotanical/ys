...
thread_t* thread_init(thread_t* thread, char* name) {
  ...
  // we used to use `strlcpy` but its not standard; perhaps we'll look at
  // `strncpy_s` if we decide to explicitly use std C11
...
bool thread_run(thread_t* thread, void* (*thread_routine)(void*), void* arg) {
  ...
  if (pthread_create(&thread->thread, &thread->attrs, thread_routine, arg) !=
      0) {
...
void thread_set_attr(thread_t* thread, bool joinable) {
  pthread_attr_setdetachstate(&thread->attrs, joinable
