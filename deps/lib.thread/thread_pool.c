 ...
 * @param routine_arg Typically application-specific data operated upon by the
 * provided routine
 *
 * @return bool Indicates whether a thread was available
 */
bool thread_pool_dispatch(thread_pool_t* pool, void* (*thread_routine)(void*),
                          void* routine_arg, bool block_caller) {
...
bool thread_pool_exec(thread_t* thread) {
  ...
  if (!thread->thread_created) {
    ...
    // if the thread is already created, it is in a blocked state - thus, we
    // resume it
