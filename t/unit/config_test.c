...
void reset_server_config() {
  server_config.log_file = NULL;
  server_config.log_level = DEFAULT_LOG_LEVEL;
  server_config.num_threads = DEFAULT_NUM_THREADS;
  server_config.port_num = DEFAULT_PORT_NUM;
}

void test_config_defaults() {
  is(server_config.log_file, NULL, "log file is NULL by default");
  is(server_config.log_level, DEFAULT_LOG_LEVEL, "default log level is set");
  ok(server_config.num_threads == DEFAULT_NUM_THREADS,
     "default number of threads is set");
  ok(server_config.port_num == DEFAULT_PORT_NUM, "default port number is set");
}
...
void test_parse_config_ok() {
  ...
  is(server_config.log_file, "server.log",
     "log file is what's specified in config");
  is(server_config.log_level, "debug",
     "log level is what's specified in config");
  ok(server_config.num_threads == 4,
     "number of threads is what's specified in config");
  ok(server_config.port_num == 8000,
     "port number is what's specified in config");
}
...
void test_parse_config_ok_empty() {
  ...
  is(server_config.log_file, NULL, "log file is default when config is empty");
  is(server_config.log_level, DEFAULT_LOG_LEVEL,
     "log level is default when config is empty");
  ok(server_config.num_threads == DEFAULT_NUM_THREADS,
     "number of threads is default when config is empty");
  ok(server_config.port_num == DEFAULT_PORT_NUM,
...
void test_parse_config_ok_no_config() {
  ...
  is(server_config.log_file, NULL, "log file is default when no config");
  is(server_config.log_level, DEFAULT_LOG_LEVEL,
...
void test_parse_config_err_missing_value() {
  ...
  is(server_config.log_file, NULL,
     "log file is default when parse_config fails");
  is(server_config.log_level, DEFAULT_LOG_LEVEL,
